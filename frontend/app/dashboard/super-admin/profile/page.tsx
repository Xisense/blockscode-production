...
export default function SuperAdminProfilePage() {
    ...
    const handleChangePassword = async () => {
        ...
        try {
            ...
            toast("Logging you out. Please sign in with your new password.", "success", "System Keys Rotated");
            setTimeout(() => {
                AuthService.logout();
            }, 2000);
        } catch (error: any) {
            toast(
                error.message === 'INVALID_CURRENT_PASSWORD'
                    ? "Current root password is incorrect"
                ...
                <div className="bg-white rounded-3xl border border-slate-100 p-8 mb-8 shadow-sm flex flex-col md:flex-row items-center gap-8 animate-in fade-in slide-in-from-top-4 duration-500">
                    <div className="relative group">
                        <div
                            ...
                            className="w-32 h-32 rounded-3xl bg-gradient-to-br from-slate-800 to-slate-950 flex items-center justify-center text-white text-5xl font-black shadow-2xl shadow-slate-200 cursor-pointer overflow-hidden transition-transform hover:scale-105 active:scale-95"
                        >
                            {avatar ? (
                    ...
                    <div className="flex-1 text-center md:text-left">
                        ...
                        <div className="flex flex-wrap justify-center md:justify-start gap-3">
                            <span className="px-4 py-1.5 bg-slate-900 text-white text-[11px] font-black uppercase tracking-widest rounded-full border border-slate-800">
                            ...
                            <span className="px-4 py-1.5 bg-slate-100 text-slate-500 text-[11px] font-black uppercase tracking-widest rounded-full">
                                ID: #{userData?.id?.substring(0, 8) || "N/A"}
                            </span>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="flex-1 w-full bg-white rounded-3xl border border-slate-100 p-10 shadow-sm min-h-[500px] animate-in slide-in-from-right-4 duration-500">
                        {activeTab === 'general' ? (
                            <div className="space-y-8 max-w-2xl">
                                <div>
                                    <h2 className="text-xl font-black text-slate-800 mb-2">Root Administrative Profile</h2>
                                    <p className="text-sm font-medium text-slate-400">Global system oversight credentials.</p>
                                </div>

                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        <label className="text-[11px] font-black uppercase tracking-widest text-slate-400">Display Name</label>
                                        <input
                                            type="text"
                                            value={name}
                                            onChange={(e) => setName(e.target.value)}
                                            className="w-full h-12 bg-slate-50 border border-slate-100 rounded-xl px-4 text-sm font-bold text-slate-700 focus:outline-none focus:ring-2 focus:ring-slate-100 focus:border-slate-800 transition-all"
                                        />
                                    </div>

                                    <div className="space-y-2">
                                        <label className="text-[11px] font-black uppercase tracking-widest text-slate-400">Root Email</label>
                                        <input
                                            type="email"
                                ...
                                <button
                                    ...
                                    className="px-8 py-3 bg-slate-950 text-white rounded-xl text-sm font-black shadow-lg hover:scale-105 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                                >
                                    {saving ? "Syncing..." : "Sync Global Profile"}
                                </button>
                            </div>
                        ) : (
                            <div className="space-y-8 max-w-2xl">
                                <div>
                                    <h2 className="text-xl font-black text-slate-800 mb-2">Root Security</h2>
                                    <p className="text-sm font-medium text-slate-400">Critical system access controls.</p>
                                </div>

                                <div className="space-y-6">
                                    <div className="space-y-2">
                                        <label className="text-[11px] font-black uppercase tracking-widest text-slate-400">Current Root Password</label>
                                        <input
                                            type="password"
                                            placeholder="••••••••"
                                            value={currentPass}
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        <div className="space-y-2">
                                            <label className="text-[11px] font-black uppercase tracking-widest text-slate-400">New Root Password</label>
                                            <input
                                                type="password"
                                                placeholder="••••••••"
                                                value={newPass}
                                        ...
                                        <div className="space-y-2">
                                            <label className="text-[11px] font-black uppercase tracking-widest text-slate-400">Confirm Root Password</label>
                                            <input
                                                type="password"
                                                placeholder="••••••••"
                                                value={confirmPass}
                                ...
                                <button
                                    ...
                                    disabled={saving}
                                    className="px-8 py-3 bg-red-600 text-white rounded-xl text-sm font-black shadow-lg hover:scale-105 active:scale-95 transition-all disabled:opacity-50"
                                >
                                    {saving ? "Rotating..." : "Rotate System Keys"}
                                </button>
                            </div>
                        )}
                    </div>
